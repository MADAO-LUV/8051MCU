...
void main()
{
	DS18B20_ConvertT(); //上电先转换一次温度，防止第一次读数数据错误
	Delay(1000); //延迟1s 等待转换完成
	...
	while(1)
	{
		...
		LCD_ShowNum(1,8,(unsigned long)(TShow*100)%100,2); //显示小数的前两位
		...
		if(KeyNum)
		{
			...
			if(KeyNum==2)  //K2按键，THigh自降
			{
				THigh--;
				if(THigh<=TLow){THigh++;}
			}
			if(KeyNum==3) //K3按键，TLow自加
			{
				TLow++;
				if(TLow>=THigh){TLow--;}
			}
			...
			LCD_ShowSignedNum(2,12,TLow,3);
			AT24C02_WriteByte(0,THigh); //写入到AT24C02中保存起来
			Delay(5);
